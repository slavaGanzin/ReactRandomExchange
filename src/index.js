...
function CurrencyTable (props) {
    ...
    return <div className="CurrencyTable">
        ...
        {props.currencies.map(({name, key}) => <div key={key} className={`${selected === key
            ? "selected "
            : ""}row`} onClick={() => setSelected(key)}>
            <div>{name}</div>
...
function App () {
    ...
    return (
        <div className={`App ${!Object.keys(rates).length || !currencies.length
            ? "hidden"
            : ""}`}>
