...
function CurrencySelect(props) {
  return (
    <select name="CurrencySelect" value={props.selected} onChange={props.onChange}>
      {currencies.map(({name, key}) => (<option key={key} value={key}>{name}</option>))}
    </select>
  )
}
function CurrencyExchangeForm() {
  ...
  return (
    <div className='CurrencyExchangeForm'>
      ...
      <div className='row'>
        <h3>From</h3>
        <CurrencySelect selected={fromCurrency} onChange={event => setFromCurrency(event.target.value)} />
      </div>
      <div className='row'>
      ...
      <div className='row'>
        <h3>To</h3>
        <CurrencySelect selected={toCurrency} onChange={event => setToCurrency(event.target.value)} />
      </div>
    </div>
  )
}
